...
BASE_URL="https://github.com/cognitive-engineering-lab/repo-quest/releases/latest/download"
...
pick_target() {
  ...
  if [ -n "$1" ]; then
    cargo install repo-quest --locked --git https://github.com/cognitive-engineering-lab/repo-quest/ --rev $1
    return
  elif [ "$OS" = "Linux" ]; then
    if [ "$ARCH" = "x86_64" ]; then
...
echo 'RepoQuest installation is complete.'